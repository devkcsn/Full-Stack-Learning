...
// Create career (admin only - for seeding data)
...
router.post('/seed', async (req, res) => {
  try {
    ...
    if (existingCareers > 0) {
      return res.status(400).json({ message: 'Careers already seeded' });
    }
    ...
    res.status(201).json({ message: 'Careers seeded successfully', count: careers.length });
  } catch (error) {
